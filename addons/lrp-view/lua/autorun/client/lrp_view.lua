...
hook.Add("CalcView", "DeathView", function(ply, origin, angles, fov)
    ...
    if ply:Alive() and weapon and weapon:IsValid() and table.HasValue(BlackListWeapons, weapon:GetClass()) then return end
...
hook.Add("PostDrawTranslucentRenderables", "lrpcrosshair", function()
    ...
    if not ply:InVehicle() then
        if IsValid(wep) then
            if wep.LRPGuns and ply:KeyDown(IN_ATTACK2) and not wep:GetReloading() then
    ...
    if not ply:InVehicle() and ply:GetViewEntity() == ply then
        ...
        if IsValid(wep) then
            if IsValid(trace.Entity) and table.HasValue(HandMaterialObjects, trace.Entity:GetClass()) and ply:GetShootPos():Distance(trace.HitPos) < 90 and table.HasValue(HandMaterialWeapons, wep:GetClass()) then
...
hook.Add("PostDrawHUD", "BlackScreen", function()
    ...
    if wep.LRPGuns and handview and ply:KeyDown(IN_ATTACK2) and ply:IsValid() and ply:Alive() and IsValid(ply) and IsValid(wep) and hand then
        ...
        if hullTrace.Hit and IsValid(hullTrace.Entity) and hullTrace.Entity:GetClass() ~= "player" and hullTrace.Entity:GetClass() ~= "gmod_sent_vehicle_fphysics_base" and LocalPlayer():GetMoveType() ~= MOVETYPE_NOCLIP then
