...
SWEP.PrintName = "Полицейский щит"
SWEP.Slot = 4
SWEP.SlotPos = 2
SWEP.DrawAmmo = false
...
SWEP.VElements = {
	["shield"] = { type = "Model", model = "models/bshields/rshield.mdl", bone = "Bip01 R Hand", rel = "", pos = Vector(1, -7, 2), angle = Angle(0, 0, 0), size = Vector(1.014, 1.014, 1.014), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_shield"] = { type = "Model", model = "models/bshields/rshield.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(0, 8, 5), angle = Angle(0, -10, 165), size = Vector(1.2, 1.2, 1.2), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function shouldBeBlocked(target,dmginfo)
	if target:IsValid() and target:IsPlayer() and IsValid(target:GetActiveWeapon()) and target:GetActiveWeapon():GetClass() == "lrp_shield" then
		...
		if dmginfo:GetAttacker():IsValid() then
			if difangles.p <= 40 and difangles.y <= 45 then
				return true
			end
		end
	end
...
function SWEP:Initialize()
	...
	if table.Count(ents.FindByClass("lrp_shield")) <= 1 then
...
function SWEP:OnRemove()
	if table.Count(ents.FindByClass("lrp_shield")) <= 0 then
...
if CLIENT then
	...
	hook.Add("InputMouseApply", "SetupMove", function( ucmd )
		...
		down = math.Clamp(-40, -90, -40)
		up = math.Clamp(35, 35, 90)

		if IsValid(wep) and wep:GetClass() == "lrp_shield" then
